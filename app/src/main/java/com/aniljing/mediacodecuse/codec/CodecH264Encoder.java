...
public class CodecH264Encoder {
    ...
    public int h264Encoder(byte[] input) {
        if (input == null || mMediaCodecVideo == null) {
            LogUtils.e("编码错误:" + "input==null is" + (input == null) + "encoder==null is " + (mMediaCodecVideo == null));
            return 0;
        }
        yuv420 = input;
        int pos = 0;
        ...
        try {
            ...
            if (inputBufferIndex >= 0) {
                ...
                long nts = computePresentationTime(m_nCount);
                //把数据放入可用的缓冲区，通知编码器编码
                mMediaCodecVideo.queueInputBuffer(inputBufferIndex, 0, yuv420.length, nts, 0);
                m_nCount++;
            } else {
                LogUtils.e(TAG, "编码输入缓冲 inputBufferIndex--->" + inputBufferIndex);
            }
            MediaCodec.BufferInfo bufferInfo = new MediaCodec.BufferInfo();
            //获取编码好的索引，相机预览帧率为25，故设置超时为40ms，若设置0则outputBufferIndex可能小于0导致马赛克问题
            int outputBufferIndex = mMediaCodecVideo.dequeueOutputBuffer(bufferInfo, 40000);

            if (outputBufferIndex < 0) {
                LogUtils.e("编码输出缓冲 outputBufferIndex--->" + outputBufferIndex);
            }
            ...
            while (outputBufferIndex >= 0) {
                ...
                mMediaCodecVideo.releaseOutputBuffer(outputBufferIndex, false);
                outputBufferIndex = mMediaCodecVideo.dequeueOutputBuffer(bufferInfo, 0);
            }
            ...
            t.printStackTrace();
            // Log.e("offerEncoder",   " 777777777777"  + t.toString());
        }
        return pos;
    }
